...
using App.Library.WebAPI.MultipartDataMediaFormatter.Infrastructure.Logger;
...
namespace App.Library.WebAPI.MultipartDataMediaFormatter.Converters
{
    public class FormDataToObjectConverter
    {
        ...
        private bool TryGetFromFormData(Type destinitionType, string propertyName, out object propValue)
        {
            ...
            if (destinitionType == typeof(RawFile))
            {
                RawFile rawFile;
                if (SourceData.TryGetValue(propertyName, out rawFile))
                {
                    propValue = rawFile;
