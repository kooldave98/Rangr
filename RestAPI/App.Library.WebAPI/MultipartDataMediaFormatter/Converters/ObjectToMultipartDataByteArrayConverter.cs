...
using App.Library.WebAPI.MultipartDataMediaFormatter.Infrastructure;
using App.Library.WebAPI.MultipartDataMediaFormatter.Infrastructure.Extensions;

namespace App.Library.WebAPI.MultipartDataMediaFormatter.Converters
{
        ...
        private byte[] GetMultipartFormDataBytes(List<KeyValuePair<string, object>> postParameters, string boundary)
        {
            ...
            using (var formDataStream = new System.IO.MemoryStream())
            {
                ...
                foreach (var param in postParameters)
                {
                    ...
                    if (param.Value is RawFile)
                    {
                        RawFile rawFileToUpload = (RawFile)param.Value;
                        ...
                        string header =
                            string.Format("--{0}\r\nContent-Disposition: form-data; name=\"{1}\"; filename=\"{2}\"\r\nContent-Type: {3}\r\n\r\n",
                                boundary,
                                param.Key,
                                rawFileToUpload.FileName ?? param.Key,
                                rawFileToUpload.MediaType ?? "application/octet-stream");

                        formDataStream.Write(encoding.GetBytes(header), 0, encoding.GetByteCount(header));

                        // Write the file data directly to the Stream, rather than serializing it to a string.
                        formDataStream.Write(rawFileToUpload.Buffer, 0, rawFileToUpload.Buffer.Length);
                    }
                    else
                    {
                        string objString = "";
